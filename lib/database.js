...
export async function getAssessmentsOverview(studentRefID) {
  return await runQuery(
    Call(Function("get_assessments_overview_by_studentRefID"), studentRefID)
  )
}

export async function getAssessmentDetailsByAssessmentRefID(id) {
  const check = await runQuery(Exists(Ref(Collection("assessments"), id)));

  if (!check.result) return {
    error: true,
    result: false
  }
  else return await runQuery(
